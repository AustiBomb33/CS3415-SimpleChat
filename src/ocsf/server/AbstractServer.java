...
* runs as a thread. When a ocsf.client is thus connected to the
...
public abstract class AbstractServer implements Runnable
{
  ...
  /**
   * The thread group associated with ocsf.client threads. Each member of the
...
// INSTANCE METHODS *************************************************
   ...
   * Any exception thrown while closing a ocsf.client is ignored.
  ...
  final synchronized public void close() throws IOException
  {
    ...
    {
      // Close the ocsf.client sockets of the already connected clients
  ...
  /**
   * Sends a message to every ocsf.client connected to the server.
   ...
   * sending the message to a particular ocsf.client is ignored.
   *
...
// ACCESSING METHODS ------------------------------------------------
   ...
   * ocsf.client connections. This can be used by
...
// RUN METHOD -------------------------------------------------------
  ...
  final public void run()
  {
    ...
    try
    {
      // Repeatedly waits for a new ocsf.client connection, accepts it, and
      ...
      while(!readyToStop)
      {
        try
        {
          ...
          // When a ocsf.client is accepted, create a thread to handle
...
// METHODS DESIGNED TO BE OVERRIDDEN BY CONCRETE SUBCLASSES ---------
  ...
  /**
   * Hook method called each time a new ocsf.client connection is
   * accepted. The default implementation does nothing.
   * @param client the connection connected to the ocsf.client.
   */
  protected void clientConnected(ConnectionToClient client) {}

  /**
   * Hook method called each time a ocsf.client disconnects.
   * The default implementation does nothing. The method
   * may be overridden by subclasses but should remains synchronized.
   *
   * @param client the connection with the ocsf.client.
   */
   ...
   * synchronized.
   *
   * @param client the ocsf.client that raised the exception.
  ...
  /**
   * Handles a command sent from one ocsf.client to the server.
   ...
   * @param client the connection connected to the ocsf.client that
   *  sent the message.
   */
...
// METHODS TO BE USED FROM WITHIN THE FRAMEWORK ONLY ----------------
  ...
  /**
   * Receives a command sent from the ocsf.client to the server.
